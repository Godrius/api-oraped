...
/**
 * Finalidade:
 * Administrar os tamanhos definidos no escopo da categoria pelo WhatsApp.
 *
 * Aplicação:
 * - ativa/desativa grade de tamanhos da categoria
 * - cadastra opções de tamanho da categoria
 * - lista e gerencia opções de tamanho
 * - altera descrição e status das opções
 *
 * Utilização:
 * Deve ser chamado pelo roteamento administrativo de tamanhos por categoria.
 */
@Service
@RequiredArgsConstructor
public class AdminTamanhoCategoriaService {
    ...
    public AdministradorWhatsappResultados.ResultadoAdmin montarMenuTamanhosCategoria(
        ...
        GradeTamanho grade = gradeTamanhoService.buscarGradeAtivaDaCategoria(idCategoria);

        List<OpcaoTamanhoResponseDTO> tamanhos = grade == null
            ? List.of()
            : gradeTamanhoService.listarOpcoes(grade.getId(), false);
    ...
    public AdministradorWhatsappResultados.ResultadoAdmin ativarTamanhosCategoria(
        ...
        gradeTamanhoService.buscarOuCriarGradeDaCategoria(estabelecimento, categoria.getId());

        String corpo =
            "✅ Tamanhos ativados.\n\n" +
                "Categoria: *" + uiHelper.msg().trunc(uiHelper.msg().safe(categoria.getNome()), 80) + "*\n\n" +
                "A partir de agora, os produtos desta categoria usarão os tamanhos próprios da categoria.";
    ...
    public AdministradorWhatsappResultados.ResultadoAdmin desativarTamanhosCategoria(
        ...
        GradeTamanho grade = gradeTamanhoService.buscarGradeAtivaDaCategoria(categoria.getId());

        if (grade != null) {
            gradeTamanhoService.atualizarStatusGrade(grade.getId(), false);
        }
    ...
    public AdministradorWhatsappResultados.ResultadoAdmin montarMenuOpcoesTamanhoCategoria(
        ...
        List<OpcaoTamanhoResponseDTO> opcoes = gradeTamanhoService.listarOpcoes(grade.getId(), false);
    ...
    public AdministradorWhatsappResultados.ResultadoAdmin iniciarCadastroOpcaoTamanho(
        ...
        GradeTamanho grade = gradeTamanhoService.buscarOuCriarGradeDaCategoria(
            estabelecimento,
            categoria.getId()
        );
        ...
        String corpo =
            "➕ *Novo tamanho da categoria*\n\n" +
                "*" + uiHelper.msg().trunc(uiHelper.msg().safe(categoria.getNome()), 80) + "*\n\n" +
                montarTextoTamanhosCadastrados(tamanhosCadastrados, null) +
                "\n\n" +
    ...
    public AdministradorWhatsappResultados.ResultadoAdmin concluirCadastroOpcaoTamanho(
        ...
        // A opção define apenas o tamanho; preço é configurado no produto.
        ...
        String corpo =
            "✅ Tamanho *" + uiHelper.msg().trunc(uiHelper.msg().safe(nome), 80) + "* cadastrado.\n\n" +
                montarTextoTamanhosCadastrados(tamanhosAtualizados, nome);
    ...
    public String montarTextoTamanhosCadastrados(
    ...
    private OpcaoTamanho validarContextoOpcaoTamanho(
        ...
        GradeTamanho gradeCategoria = gradeTamanhoService.buscarGradeAtivaDaCategoria(idCategoria);

        if (gradeCategoria == null || !Objects.equals(gradeCategoria.getId(), opcao.getGrade().getId())) {
