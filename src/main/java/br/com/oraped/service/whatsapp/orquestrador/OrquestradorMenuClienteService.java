...
import br.com.oraped.domain.produto.GrupoComplementoCategoriaProduto;
...
import br.com.oraped.repository.produto.ComplementoRepository;
import br.com.oraped.repository.produto.GrupoComplementoCategoriaProdutoRepository;
...
public class OrquestradorMenuClienteService {
	        ...
	        .map(p -> {
	            ...
	            if (StringUtils.hasText(descricao)) {
	                ...
	                if (texto.length() > limiteDescricao) {
	                    if (limiteDescricao > 3) {
	                        descricaoFinal = texto.substring(0, limiteDescricao - 3) + "...";
	                    } else {
	                        descricaoFinal = texto.substring(0, limiteDescricao);
	                    }
	                } else {
    ...
    public MensagemWhatsappSaidaDTO montarListaQuantidades(
	    ...
	    String precoUnitario = msg.formatarMoeda(produto.getPreco());
    ...
    public MensagemWhatsappSaidaDTO montarListaComplementosEmMontagem(
    	...
    	cabecalho.append("*").append(msg.safe(produto.getNome())).append("*\n\n")
    	    .append("Preço: ").append(msg.formatarMoeda(produto.getPreco())).append("\n\n")
    	    .append("Complementos:\n")
